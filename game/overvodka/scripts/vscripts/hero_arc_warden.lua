...
LinkLuaModifier("modifier_imba_arc_warden_magnetic_field_thinker_evasion", "hero_arc_warden", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_arc_warden_magnetic_field_attack_speed", "hero_arc_warden", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_arc_warden_magnetic_field_evasion", "hero_arc_warden", LUA_MODIFIER_MOTION_NONE)

LinkLuaModifier("modifier_imba_arc_warden_spark_wraith_thinker", "hero_arc_warden", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_imba_arc_warden_spark_wraith_purge", "hero_arc_warden", LUA_MODIFIER_MOTION_NONE)
...
function modifier_imba_arc_warden_magnetic_field_thinker_attack_speed:GetAuraDuration()				return 1.0 end
...
function modifier_imba_arc_warden_magnetic_field_thinker_attack_speed:GetAuraSearchTeam()			return DOTA_UNIT_TARGET_TEAM_BOTH end
...
function modifier_imba_arc_warden_magnetic_field_attack_speed:OnCreated()
	...
	elseif self:GetAuraOwner() and self:GetAuraOwner():HasModifier("modifier_imba_arc_warden_magnetic_field_thinker_attack_speed") then
		...
		self.steal	= self:GetAuraOwner():FindModifierByName("modifier_imba_arc_warden_magnetic_field_thinker_attack_speed").steal
		self.stolen = self:GetParent():GetStrength() * self.steal * 0.01
		if self:GetParent():IsRealHero() and self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
			k = k + 1
			if self:GetCaster():HasScepter() then
...
function modifier_imba_arc_warden_magnetic_field_attack_speed:OnDestroy()
	k = k - 1
	if self:GetCaster():HasScepter() then
		k = k - 1
	end
...
function modifier_imba_arc_warden_magnetic_field_attack_speed:GetModifierBonusStats_Strength()
	if self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() and self:GetCaster():HasScepter() then
		return -self.stolen
	end
end
function modifier_imba_arc_warden_magnetic_field_attack_speed:GetModifierBonusStats_Agility()
	if self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() and self:GetCaster():HasScepter() then
		return -self.stolen
	end
end
function modifier_imba_arc_warden_magnetic_field_attack_speed:GetModifierBonusStats_Intellect()
	if self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
		return -self.stolen
	end
...
-----------------------------------------------------
function modifier_imba_arc_warden_magnetic_field_evasion:IsHidden() return true end
