function Vacuum( keys )
	...
	if caster:GetUnitName() == "npc_dota_hero_rubick" then
		ability_level = caster:GetLevel() / 3
	else
		ability_level = caster:FindAbilityByName("invoker_wex_lua"):GetLevel() - 1
	end

	-- Ability variables
	local damage = ability:GetLevelSpecialValueFor("damage", ability_level) / 15
