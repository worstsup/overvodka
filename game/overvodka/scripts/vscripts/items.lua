...
function COverthrowGameMode:SpecialItemAdd( event )
	...
	if nLeaderKills <= 5 then
		spawnedItem = t1
	elseif nLeaderKills > 5 and nLeaderKills <= 10 then
	...
	elseif nLeaderKills > 10 and nLeaderKills <= 20 then
		if ownerTeam == leader and ( self.leadingTeamScore - self.runnerupTeamScore > 3 ) then
			spawnedItem = t3
		elseif ownerTeam == lastPlace then
			spawnedItem = t4
	...
	elseif nLeaderKills > 20 and nLeaderKills <= 30 then
		if ownerTeam == leader and ( self.leadingTeamScore - self.runnerupTeamScore > 3 ) then
			spawnedItem = t4
		elseif ownerTeam == lastPlace then
			spawnedItem = t5
