function StartBrewing( event )
	...
	if caster:HasScepter() then
		ability:ApplyDataDrivenModifier(caster, caster, "modifier_unstable_concoction_brewing_scepter", {duration = ability:GetLevelSpecialValueFor( "brew_explosion_scepter", ability:GetLevel() - 1 ) })
	else
		ability:ApplyDataDrivenModifier(caster, caster, "modifier_unstable_concoction_brewing", {duration = ability:GetLevelSpecialValueFor( "brew_explosion", ability:GetLevel() - 1 ) })
	end
	-- Play the sound, which will be stopped when the sub ability fires
	caster:EmitSound("drunk")
...
function UpdateTimerParticle( event )
    ...
    if caster:HasScepter() then
		brew_explosion = brew_explosion + 4.0
	end
...
function EndBrewing( event )
	...
	if caster:HasScepter() then
		caster:RemoveModifierByName("modifier_unstable_concoction_brewing_scepter")
	else
