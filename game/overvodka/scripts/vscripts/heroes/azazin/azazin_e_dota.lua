LinkLuaModifier("modifier_azazin_e_dota", "heroes/azazin/azazin_e_dota", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_azazin_e_dota_caster", "heroes/azazin/azazin_e_dota", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_azazin_e_dota_enemy", "heroes/azazin/azazin_e_dota", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_azazin_e_dota_debuff", "heroes/azazin/azazin_e_dota", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_generic_stunned_lua", "modifier_generic_stunned_lua", LUA_MODIFIER_MOTION_NONE)

azazin_e_dota = class({})
k = 0
function azazin_e_dota:Precache(context)
...
function azazin_e_dota:GetIntrinsicModifierName() 
    return "modifier_azazin_e_dota"
end

modifier_azazin_e_dota = class({})

function modifier_azazin_e_dota:IsHidden() 
    return true
end
function modifier_azazin_e_dota:IsPurgable() 
    return false 
end

function modifier_azazin_e_dota:DeclareFunctions()
    return {
        MODIFIER_EVENT_ON_ATTACK_LANDED,
    }
end

function modifier_azazin_e_dota:OnCreated()
    if not IsServer() then return end
end

function modifier_azazin_e_dota:OnAttackLanded(params)
    if not IsServer() then return end
    ...
    if target:HasModifier("modifier_azazin_e_dota_enemy") then
        local enemyMod = target:FindModifierByName("modifier_azazin_e_dota_enemy")
        enemyMod:IncrementStackCount()
        enemyMod:SetDuration(counter_duration, true)
        if enemyMod:GetStackCount() >= required_hits then
            enemyMod:Destroy()
            if self:GetParent():HasScepter() then
                self:GetParent():AddNewModifier(self:GetParent(), ability, "modifier_azazin_e_dota_caster", { duration = effect_duration })
            end
            target:AddNewModifier(self:GetParent(), ability, "modifier_generic_stunned_lua", { duration = stun_duration })
            target:AddNewModifier(self:GetParent(), ability, "modifier_azazin_e_dota_debuff", { duration = effect_duration })
        ...
        target:AddNewModifier(self:GetParent(), ability, "modifier_azazin_e_dota_enemy", { duration = counter_duration })
        local enemyMod = target:FindModifierByName("modifier_azazin_e_dota_enemy")
        if enemyMod then
...
modifier_azazin_e_dota_enemy = class({})

function modifier_azazin_e_dota_enemy:IsHidden() 
    return false 
end

function modifier_azazin_e_dota_enemy:IsDebuff() 
    return true 
end

function modifier_azazin_e_dota_enemy:IsPurgable() 
    return false 
end

function modifier_azazin_e_dota_enemy:RemoveOnDeath() 
    return true 
end

function modifier_azazin_e_dota_enemy:OnCreated(kv)
    if not IsServer() then return end
...
function modifier_azazin_e_dota_enemy:OnStackCountChanged(previous_stacks)
    if not IsServer() then return end
    if not self.particle then return end
    ParticleManager:SetParticleControl(self.particle, 1, Vector(0, self:GetStackCount(), 0))
end

function modifier_azazin_e_dota_enemy:OnRefresh(kv)
    if not IsServer() then return end
    self:IncrementStackCount()
end

modifier_azazin_e_dota_caster = class({})

function modifier_azazin_e_dota_caster:IsPurgable() 
    return false 
end

function modifier_azazin_e_dota_caster:OnCreated()
    if not IsServer() then return end
...
modifier_azazin_e_dota_debuff = class({})
function modifier_azazin_e_dota_debuff:IsDebuff() return true end
function modifier_azazin_e_dota_debuff:IsPurgable() return true end

function modifier_azazin_e_dota_debuff:OnCreated()
    if not IsServer() then return end
    self.interval = 0.5
    self:StartIntervalThink(self.interval)
end

function modifier_azazin_e_dota_debuff:OnIntervalThink()
    if not IsServer() then return end
...
function modifier_azazin_e_dota_debuff:GetEffectAttachType()
