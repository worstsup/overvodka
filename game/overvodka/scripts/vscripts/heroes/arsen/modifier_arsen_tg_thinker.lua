modifier_arsen_tg_thinker = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_arsen_tg_thinker:IsHidden()
	return true
end

--------------------------------------------------------------------------------
-- Initializations
function modifier_arsen_tg_thinker:OnCreated( kv )
	-- references
...
function modifier_arsen_tg_thinker:OnRefresh( kv )
	
end

function modifier_arsen_tg_thinker:OnRemoved()
	if not IsServer() then return end
	-- stop effects
end

function modifier_arsen_tg_thinker:OnDestroy()
	if not IsServer() then return end
...
--------------------------------------------------------------------------------
-- Interval Effects
function modifier_arsen_tg_thinker:OnIntervalThink()
	if self.phase_delay then
		...
		self:GetParent():AddNewModifier(
			...
			"modifier_arsen_tg_wall_aura", -- modifier name
			{  } -- kv
		)
		...
		self:GetParent():AddNewModifier(
			self:GetCaster(), -- player source
			self:GetAbility(), -- ability source
			"modifier_arsen_tg_spear_aura", -- modifier name
			{  } -- kv
		)
		...
		self:GetParent():AddNewModifier(
			self:GetCaster(), -- player source
			self:GetAbility(), -- ability source
			"modifier_arsen_tg_projectile_aura", -- modifier name
			{  } -- kv
		)
...
function modifier_arsen_tg_thinker:SummonBlockers()
	-- init data
	...
	for i=0,count-1 do
		...
		local callback = function( unit )
			...
			unit:AddNewModifier(
				...
				self:GetAbility(), -- ability source
				"modifier_arsen_tg_blocker", -- modifier name
				{
