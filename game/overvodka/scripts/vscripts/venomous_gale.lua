function VenomousGaleImpact( keys )
    ...
    local percent = ability:GetLevelSpecialValueFor("percent", ability_level) / 100
    local duration = ability:GetLevelSpecialValueFor("duration", ability_level)
    local damage = 0
    damage = damage + target_maxHealth * percent
    ...
    ApplyDamage({victim = keys.target, attacker = keys.caster, damage = damage, damage_type = DAMAGE_TYPE_PURE,})
